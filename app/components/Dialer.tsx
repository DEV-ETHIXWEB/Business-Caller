  ...
  return (
    <div className="relative flex h-dvh w-full overflow-hidden bg-gradient-to-br from-[#F7F2F1] via-white to-[#F5EFEE] dark:from-[#0c0d10] dark:via-[#120a0b] dark:to-black">
      ...
      <nav className={NAV_RAIL_CLASS}>
        <Image src="/ethixweb-logo.png" alt="Ethixweb" width={32} height={32} className="mb-2 h-6 w-auto dark:invert" />
        <button
          type="button"
          onClick={() => {
            playTap();
          ...
          <div className={CARD_CLASS}>
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold tracking-wide text-slate-900 dark:text-white">
                {callStatus === "ready" ? "New call" : overlayName ?? phoneNumber}
              </h2>
              {callStatus === "ready" && (
                <button
                  type="button"
            ...
            <div className="mt-6 flex items-center justify-center gap-2 border-t border-slate-900/5 pt-4 dark:border-white/5">
              ...
              />
              <p className="text-sm font-medium text-slate-600 dark:text-slate-300">Status: {statusLabel}</p>
            </div>
          </div>
        </div>
      )}
